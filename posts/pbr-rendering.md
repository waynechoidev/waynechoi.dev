...
The pre-filtered environment map serves as a pre-computed representation of how light is reflected on the surface of an object across varying roughness levels. This map captures the reflection properties and surface characteristics, particularly defining glossiness or specular highlights of the object. It encompasses the simulation of both direct and indirect specular highlights present on the surface of objects. The code for generating the pre-filtered environment map from the Environment Map was directly adopted from [Learn OpenGL](https://learnopengl.com/).
